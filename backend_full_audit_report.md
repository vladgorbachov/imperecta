...
- В `backend/alembic/env.py` перед `run_migrations()` добавлен runtime guard:
  - проверка наличия `public.dim_date` как индикатора фактически применённой v2 схемы;
  - при отсутствии таблицы выполняется `DELETE FROM alembic_meta.alembic_version`;
  - затем Alembic повторно применяет `001_v2_schema` -> `002_v2_additions`.
- В `context.configure(...)` для online migrations включён `compare_type=True`.
- Риск "stamped head without DDL" закрыт: если версия в `alembic_meta.alembic_version` записана ошибочно, система автоматически форсирует реальный прогон миграций.
...
### Миграции
- Цепочка: `001_v2_schema` → `002_v2_additions` → `003_fix_users_columns` → `004_fix_real_state` (head). Старая цепочка Alembic 001–016 из репозитория удалена.
...
### `versions/002_v2_additions.py` (head)
