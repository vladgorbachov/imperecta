"""Fuel ingestion provider. Internal pipeline data. Configurable HTTP source."""
...
class FuelHttpAdapter(CommoditiesProviderAdapter):
    """
    Fuel ingestion adapter. Internal pipeline data from configurable URL.
    Expects JSON array with symbol, price, change_24h, unit, name.
    ...
    async def fetch(self) -> list[NormalizedCommodity]:
        """Fetch fuel/internal data. Returns empty if URL not configured."""
        ...
        if not isinstance(data, list):
            logger.warning("Fuel response not a list: %s", type(data))
            return []
        ...
        for row in data:
            try:
                ...
                logger.warning("Parse fuel row: %s", error)
                continue

        logger.info("Fuel adapter fetched %d items", len(items))
        return items
