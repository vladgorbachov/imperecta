...
Used by dashboard API and AI summary endpoints.
"""
...
class DashboardService:
    ...
    async def get_kpi(self) -> dict:
        ...
        cp_ids = [row[0] for row in cp_ids_result.all()]
        ...
        avg_price_change_24h = sum(price_changes) / len(price_changes) if price_changes else 0.0

        active_alerts_result = await self.db.execute(
            select(func.count())
            .select_from(AlertEvent)
            .join(Alert, AlertEvent.alert_id == Alert.id)
            .where(Alert.user_id == self.user_id, AlertEvent.triggered_at >= day_ago)
        )
        ...
        trend_products = ((total_products - products_week_ago) / products_week_ago * 100) if products_week_ago else 0.0
        ...
        avg_price_change_week_ago = (
            sum(price_changes_week_ago) / len(price_changes_week_ago) if price_changes_week_ago else 0.0
        )
        ...
        trend_alerts = ((active_alerts_count - alerts_week_ago) / alerts_week_ago * 100) if alerts_week_ago else 0.0
    ...
    async def get_aggregate_trend(self, period_days: int = 30, forecast_days: int = 7) -> dict:
        ...
        for i in range(period_days):
            day_start = (period_start + timedelta(days=i)).replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        last_14 = [value for value in competitors_avg[-14:] if value > 0]
        if len(last_14) >= 2:
            n = len(last_14)
            x = list(range(n))
            y = last_14
