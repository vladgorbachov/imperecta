...
from app.models.global_product import GlobalProduct
...
class DiscoveryCrawler:
    ...
    async def discover(self, marketplace: AdminMarketplace) -> DiscoveryResult:
        """
        Full discovery for one marketplace.
        """
    ...
    async def _try_sitemap(self, base_url: str, limit: int) -> list[str]:
        """
        Try /sitemap.xml → find product URLs.
        """
    ...
    async def _find_category_urls(self, base_url: str) -> list[str]:
        """
        Fetch homepage, find navigation links to category/listing pages.
        """
        async with httpx.AsyncClient(timeout=20.0, follow_redirects=True) as client:
    ...
    async def _crawl_listing_pages(
        ...
        """
        Crawl listing page + pagination. Extract product URLs.
        Stop at remaining_quota or max 50 pages.
        """
        discovered: list[str] = []
        visited: set[str] = set()
