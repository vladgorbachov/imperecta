...
After scraping, checks completeness. If required fields missing
and another HTTP layer is available → retries with that layer.
"""
...
class ScraperPool:
    """
    Priority:
    1. Decodo Web Scraping API (if DECODO_ENABLED and credentials present)
    2. httpx direct (with BeautifulSoup)
    3. Playwright headless (for JS-rendered pages)
    """
    ...
    async def scrape_product(
        ...
        custom_selectors: from admin_marketplaces (may be None/empty).
        requires_js: from admin_marketplaces.requires_js.
        """
    ...
    async def scrape_listing(
        ...
        """Scrape category/listing page for product URLs."""
        layers = self._layer_order(requires_js=requires_js)
        for layer_name in layers:
            html = await self._fetch_by_layer(layer_name, url)
            if not html:
    ...
    def _layer_order(self, requires_js: bool) -> list[str]:
        ...
        if requires_js and "playwright" in layers:
            # Keep compatibility with priority, but surface JS-capable layer sooner.
    ...
    async def _fetch_html_decodo(self, url: str) -> str | None:
        """Fetch via Decodo API. Return HTML or None on failure."""
    ...
    async def _fetch_html_httpx(self, url: str) -> str | None:
        """Fetch via httpx. Return HTML or None on failure."""
        try:
            headers = {
                "User-Agent": (
    ...
    async def _fetch_html_playwright(self, url: str) -> str | None:
        """Fetch via Playwright headless. Return HTML or None on failure."""
        try:
    ...
    def _extract_all_levels(
        ...
        """Run all extraction levels on HTML and merge results."""
        soup = BeautifulSoup(html, "html.parser")
