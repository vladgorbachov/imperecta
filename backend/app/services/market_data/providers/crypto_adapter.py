...
from app.services.market_data.dto import NormalizedCrypto
from app.services.market_data.providers.base import CryptoProviderAdapter
    ...
    async def fetch(self) -> list[NormalizedCrypto]:
        ...
        for row in data:
            try:
                ...
                market_cap = Decimal(str(mcap)) if mcap is not None else None
                items.append(
                    NormalizedCrypto(
                        symbol=symbol,
                        price=price,
                ...
                logger.warning("Parse crypto row %s: %s", row.get("id"), e)
                continue
