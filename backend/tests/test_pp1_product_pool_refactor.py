...
def test_get_categories_reads_preaggregated_view() -> None:
    """/pool/categories is the same per-marketplace active count — 6-10s per
    Products-page load as a live GROUP BY; must read mv_marketplace_stats."""
    import asyncio
    from unittest.mock import AsyncMock, MagicMock
    ...
    assert asyncio.run(svc.get_categories()) == []
    sql = str(fake_db.execute.call_args.args[0].compile(dialect=postgresql.dialect()))
    assert "mv_marketplace_stats" in sql
    assert "fact_listing" not in sql
    assert "GROUP BY" not in sql
