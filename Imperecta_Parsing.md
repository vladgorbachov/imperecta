...
**Сигнатура:** `discover(marketplace, *, deadline_monotonic=None)` — cooperative deadline (Level 3), без жёсткого `asyncio.wait_for` внутри pipeline.

1. Создаётся inner `scrape_jobs` (`job_type=discovery`).
...
| 016 | `dim_marketplace.sitemap_resume_offset` — resumable sitemap discovery (**head**) |
...
4. After deploy: `alembic upgrade head` (includes `016`) **and** run or schedule `ensure_fact_price_partitions`.  
...
1. Создать inner `ScrapeJob(job_type='discovery', marketplace_id, status='running')`, commit.
