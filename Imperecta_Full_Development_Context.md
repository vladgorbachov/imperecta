...
**Дата:** 2026-05-21 (обновлено: Parsing Administration frontend+backend)
...
## 1.1 Обновление статуса: Parsing Administration

- Backend-контур «Администрирование парсинга» завершён:
  - `ParsingAdminService` + `/api/admin/parsing/*` (superuser-only),
  - Celery task `run_full_pipeline_test` с metadata по стадиям и таймингам.
- Frontend-контур завершён в `AdminPage`:
  - вкладка «Тестовый парсинг»,
  - блоки «Тестовые маркетплейсы», «Запуск тестового пайплайна», «История тестовых запусков»,
  - детальная модалка запуска, polling статуса, toast-уведомления.
- Data layer:
  - `frontend/src/api/admin.ts` расширен parsing-admin контрактами,
  - `frontend/src/hooks/useAdmin.ts` расширен parsing-admin хуками.
- Тестовое покрытие frontend:
  - `frontend/src/pages/AdminPage.parsing.test.tsx`,
  - `frontend/src/hooks/useAdmin.parsing.test.tsx`.
- Проверки: `npm test` и `npm run build` успешны.
