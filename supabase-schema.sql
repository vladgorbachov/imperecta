-- Imperecta Database Schema for Supabase
...
ALTER TABLE "clients" ADD CONSTRAINT "clients_created_by_users_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."users"("id") ON DELETE CASCADE ON UPDATE NO ACTION;
ALTER TABLE "documents" ADD CONSTRAINT "documents_uploaded_by_users_id_fk" FOREIGN KEY ("uploaded_by") REFERENCES "public"."users"("id") ON DELETE CASCADE ON UPDATE NO ACTION;
...
CREATE POLICY "Users can insert their own profile" ON users
    FOR INSERT WITH CHECK (auth.uid()::text = supabase_user_id::text);

-- Create RLS policies for user_settings table
CREATE POLICY "Users can view their own settings" ON user_settings
    FOR SELECT USING (auth.uid()::text = (SELECT supabase_user_id::text FROM users WHERE users.id = user_settings.user_id));

CREATE POLICY "Users can update their own settings" ON user_settings
    FOR UPDATE USING (auth.uid()::text = (SELECT supabase_user_id::text FROM users WHERE users.id = user_settings.user_id));

CREATE POLICY "Users can insert their own settings" ON user_settings
    FOR INSERT WITH CHECK (auth.uid()::text = (SELECT supabase_user_id::text FROM users WHERE users.id = user_settings.user_id)); 