...
- **Сопутствующий spend-guard**: после фикса ~550k листингов реально уходят на платный бэкенд (~$1.9/1k), а лимитер был только по RPS (10/с ≈ 864k/сутки теоретически). Добавлен fleet-wide бюджетный лимит поверх существующего счётчика `proxy_provider:usage:<YYYYMMDD>`: `PROXY_PROVIDER_MONTHLY_BUDGET_USD` × `PROXY_COST_PER_1K_USD` → потолок запросов на биллинг-цикл, остаток цикла раскладывается равномерно по оставшимся дням (`budget_status_sync`; опционально жёсткий `PROXY_PROVIDER_DAILY_CAP`). Цифры — из дашборда Decodo 19.09: план Web Scraping API **$19/цикл**, продление 16-го числа (`PROXY_PROVIDER_BILLING_DAY=16`), неиспользованный баланс сгорает; фактическая цена $8.74 / 9 198 успешных = **$0.95/1k** (лист: standard+JS $0.75, premium+JS $1.50) → 20 000 запросов/цикл. Локальный счётчик `proxy_provider:usage:*` появился после начала цикла и недосчитывает ~3.3k запросов (Decodo 9 198 vs локально 5 865 на 19.09 12:20) — с 16.10 цикл начнётся с чистого счётчика; скрипт сверки `scratchpad/seed_usage.py` (запись в прод-Redis классификатором заблокирована); при исчерпании — честный скип `proxy_provider_budget` (как deadline-скип: `is_empty`, без падения на datacenter-фетч). Заодно: инфра-скипы (deadline/budget) больше **не** накручивают `failure_streak`/`consecutive_errors` — раньше deadline-скип мог довести живой листинг до деактивации (15). `/admin/parsing/proxy-usage` отдаёт `monthly_budget_usd`, `monthly_cap_requests`, `month_requests`, `daily_allowance`, `today_requests`, `cap_reached`.
